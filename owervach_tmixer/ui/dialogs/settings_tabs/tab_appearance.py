"""Appearance settings: Theme skin engine, accent presets, typography, and Tier Maker sizing."""
...
def build_appearance_tab(dialog, layout: QVBoxLayout):
    # ------------------------------------------------------------------
    # 1. Selector Central de Tema (Skin Engine)
    # ------------------------------------------------------------------
    box_theme = create_card_box("Tema Visual de la Aplicación (Skin)")
    vbox_theme = QVBoxLayout(box_theme)
    vbox_theme.setSpacing(6)

    form_theme = QFormLayout()
    form_theme.setRowWrapPolicy(QFormLayout.RowWrapPolicy.WrapLongRows)
    form_theme.setSpacing(8)

    dialog.cb_theme_skin = QComboBox()
    dialog.cb_theme_skin.setView(QListView())

    themes_registry = theme.get_theme_manager().get_registered_themes()
    for tid, (d_name, _) in themes_registry.items():
        dialog.cb_theme_skin.addItem(d_name, tid)

    dialog.cb_theme_skin.currentIndexChanged.connect(dialog._on_theme_skin_selected)
    form_theme.addRow("Motor de Diseño:", dialog.cb_theme_skin)
    vbox_theme.addLayout(form_theme)

    dialog.lbl_theme_desc = QLabel("")
    dialog.lbl_theme_desc.setStyleSheet("color: #8C92A4; font-size: 11px; font-weight: 500; padding: 2px 0 4px 0;")
    dialog.lbl_theme_desc.setWordWrap(True)
    vbox_theme.addWidget(dialog.lbl_theme_desc)

    # Ocultar selector de tema para release estable (código preservado)
    box_theme.hide()
    dialog.cb_theme_skin.setEnabled(False)
    layout.addWidget(box_theme)

    # ------------------------------------------------------------------
    # 2. Color de Acento Principal
    # ------------------------------------------------------------------
    box_accent = create_card_box("Color de Acento Primario")
    ...
    dialog.btn_accent_swatch.setFixedSize(48, 26)
    ...
    dialog.edit_hex.setMaximumWidth(85)
    ...
    dialog.edit_rgb.setMaximumWidth(95)
    ...
    grid_presets.setHorizontalSpacing(6)
    grid_presets.setVerticalSpacing(6)
    ...
    for name, hex_color in theme.PRESETS.items():
        ...
        btn.setStyleSheet(f"""
            QPushButton {{
                ...
                font-weight: 700; padding: 5px 8px; border-radius: 5px; font-size: 11px;
            }}
        """)
        ...
        grid_presets.addWidget(btn, r_idx, col)
        col += 1
        if col >= 3:
            col = 0
            r_idx += 1
    vbox_accent.addLayout(grid_presets)
    ...
    dialog.btn_reset_accent.setStyleSheet("padding: 5px 10px; font-weight: 700; font-size: 11px;")
    ...
    dialog.cb_slot_align.setSizeAdjustPolicy(QComboBox.SizeAdjustPolicy.AdjustToMinimumContentsLengthWithIcon)
    dialog.cb_slot_align.addItem("★ [Rol] [Estrella] — [Nombre] — [Habilidad] (Default)", "center")
    dialog.cb_slot_align.addItem("✦ [Estrella] — [Nombre] — [Habilidad] [Rol] (Espejado)", "center_mirrored")
    dialog.cb_slot_align.addItem("⚝ [Rol] — [Nombre] — [Estrella] [Habilidad] (Simétrico)", "center_wings")
    dialog.cb_slot_align.addItem("⮜ [Estrella] [Nombre Izq] ... [Habilidad] [Rol] (Compacto)", "left")
    ...
    dialog.chk_vsync = QCheckBox("Sincronización Vertical (VSync a 144Hz sin tearing)")
    dialog.chk_vsync.setStyleSheet("color: #FFFFFF; font-weight: 700; font-size: 11.5px;")
    form_font.addRow("Sincronización:", dialog.chk_vsync)
    layout.addWidget(box_font)

    # ------------------------------------------------------------------
    # 4. Dimensiones en Tier Maker
    # ------------------------------------------------------------------
    box_tier = create_card_box("Dimensiones en Tier Maker")
    form_tier = QFormLayout(box_tier)
    form_tier.setRowWrapPolicy(QFormLayout.RowWrapPolicy.WrapLongRows)
    form_tier.setSpacing(8)

    dialog.cb_tier_export_ratio = QComboBox()
    dialog.cb_tier_export_ratio.setView(QListView())
    dialog.cb_tier_export_ratio.addItem("16:9 (Panorámico / Redes) — Recomendado", "16:9")
    dialog.cb_tier_export_ratio.addItem("Automático (Ajuste al contenido)", "auto")
    form_tier.addRow("Formato de exportación:", dialog.cb_tier_export_ratio)

    dialog.spin_tier_hero_size = QSpinBox()
    dialog.spin_tier_hero_size.setRange(50, 120)
    dialog.spin_tier_hero_size.setValue(76)
    form_tier.addRow("Retratos de héroe (px):", dialog.spin_tier_hero_size)

    dialog.spin_tier_map_w = QSpinBox()
    dialog.spin_tier_map_w.setRange(80, 220)
    dialog.spin_tier_map_w.setValue(125)
    form_tier.addRow("Ancho tarjetas de mapas:", dialog.spin_tier_map_w)

    dialog.spin_tier_map_h = QSpinBox()
    dialog.spin_tier_map_h.setRange(45, 120)
    dialog.spin_tier_map_h.setValue(75)
    form_tier.addRow("Alto tarjetas de mapas:", dialog.spin_tier_map_h)

    dialog.spin_tier_map_font = QSpinBox()
    dialog.spin_tier_map_font.setRange(10, 24)
    dialog.spin_tier_map_font.setValue(14)
    form_tier.addRow("Texto en mapas (px):", dialog.spin_tier_map_font)

    dialog.spin_tier_player_w = QSpinBox()
    dialog.spin_tier_player_w.setRange(80, 220)
    dialog.spin_tier_player_w.setValue(125)
    form_tier.addRow("Ancho tarjetas jugadores:", dialog.spin_tier_player_w)

    dialog.spin_tier_player_h = QSpinBox()
    dialog.spin_tier_player_h.setRange(45, 120)
    dialog.spin_tier_player_h.setValue(75)
    form_tier.addRow("Alto tarjetas jugadores:", dialog.spin_tier_player_h)

    dialog.chk_tier_watermark_export = QCheckBox("Incluir marca de agua al exportar imagen")
    dialog.chk_tier_watermark_export.setStyleSheet("color: #FFFFFF; font-weight: 700; font-size: 11px;")
    form_tier.addRow("Marca de agua (Exportar):", dialog.chk_tier_watermark_export)

    dialog.chk_tier_watermark_ui = QCheckBox("Mostrar marca de agua en la interfaz del Tier Maker")
    dialog.chk_tier_watermark_ui.setStyleSheet("color: #FFFFFF; font-weight: 700; font-size: 11px;")
    form_tier.addRow("Marca de agua (UI):", dialog.chk_tier_watermark_ui)
    layout.addWidget(box_tier)

    # ------------------------------------------------------------------
    # 5. Nombres de Equipo Predeterminados
    # ------------------------------------------------------------------
